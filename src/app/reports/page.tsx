...
function ReportParams({ type, params, meta, onChange }: { type: string; params: Record<string, string>; meta: any; onChange: (k: string, v: string) => void }) {
  if (type === 'assets-by-model') return (
    <div className="grid grid-cols-2 gap-3">
      <div className="space-y-1">
        ...
        >
          <SelectTrigger><SelectValue>{params.manufacturer || 'All manufacturers'}</SelectValue></SelectTrigger>
          <SelectContent>
  ...
  if (type === 'deleted-assets') return (
    <p className="text-sm text-muted-foreground">No parameters needed — click Run Report to see all deleted assets.</p>
  )
...
function ReportOutput({ type, data, params }: { type: string; data: any; params: Record<string, string> }) {
  ...
  if (type === 'deleted-assets') {
    const assets: any[] = data.assets ?? []
    if (!assets.length) return <Empty message="No deleted assets found." />
    return (
      <ReportSection title="Deleted Assets" count={assets.length} subtitle="asset(s)">
        <table className="report-table w-full">
          <thead>
            <tr>
              <Th>Asset Tag</Th>
          ...
          <tbody>
            {assets.map((a, i) => (
              <tr key={a.id} className={i % 2 === 0 ? 'bg-secondary/30' : ''}>
                ...
                <Td>{a.building}{a.room ? ` · ${a.room.name}` : ''}</Td>
                <Td mono>{new Date(a.deletedAt).toLocaleDateString()}</Td>
                <Td>{a.deletedBy ?? '—'}</Td>
                <Td>{a.deletedReason ?? '—'}</Td>
              </tr>
            ))}
          </tbody>
        </table>
