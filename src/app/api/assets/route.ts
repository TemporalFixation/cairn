...
export async function GET(req: NextRequest) {
  ...
  if (assetTag) {
    // Check primary tag first, then secondary tags
    let asset = await prisma.asset.findUnique({ where: { assetTag }, include })
    if (!asset) {
      asset = await prisma.asset.findFirst({ where: { secondaryTags: { has: assetTag } }, include }) ?? null
    }
  ...
  if (serialNumber) {
    const asset = await prisma.asset.findUnique({ where: { serialNumber }, include })
    return NextResponse.json({ asset: asset ?? null })
  }
