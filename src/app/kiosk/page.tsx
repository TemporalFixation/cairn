...
export default function KioskPage() {
  ...
  async function handleSaveCheckout() {
    if (!selectedUser) {
      if (personQuery.trim()) setShowCreateUser(true)
      else personRef.current?.focus()
      return
    }
    const person = { name: `${selectedUser.firstName} ${selectedUser.lastName}`, id: selectedUser.id, email: selectedUser.email }
    setSaving(true)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
        <span className="text-muted-foreground text-sm font-semibold tracking-wide">Cairn · Kiosk</span>
        <div className="flex rounded-lg overflow-hidden border border-border">
          {(Object.keys(MODE_CONFIG) as Mode[]).map(m => (
            <button key={m} onClick={() => switchMode(m)}
              className={`px-4 py-2 text-xs font-semibold transition-colors ${mode === m ? 'text-white' : 'text-muted-foreground'}`}
              style={mode === m ? { backgroundColor: MODE_CONFIG[m].color } : undefined}>
              {MODE_CONFIG[m].label}
            </button>
          ))}
        </div>
        <a href="/" className="text-muted-foreground/50 hover:text-muted-foreground text-xs transition-colors">Exit</a>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-10 gap-6 max-w-2xl mx-auto w-full">
        ...
        {step === 'scan' && (
          <div className="w-full space-y-6 text-center">
            <div>
              ...
              <p className="text-muted-foreground text-sm">{cfg.hint}</p>
            </div>
            <BarcodeIcon />
            <input ref={scanRef} value={scanInput} onChange={e => setScanInput(e.target.value)} onKeyDown={handleScan}
              ...
              className="w-full text-center text-2xl font-mono px-6 py-5 rounded-xl border-2 outline-none bg-muted text-foreground"
          ...
          <div className="w-full space-y-5">
            ...
            <div className="space-y-3">
              ...
              <KField label="Manufacturer *">
                <select value={enrollForm.manufacturer} onChange={e => setEnrollForm(f => ({ ...f, manufacturer: e.target.value, model: '' }))} className="kiosk-select">
                  <option value="">Select manufacturer</option>
                  {manufacturers.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
              </KField>
              <KField label="Model *">
                <select value={enrollForm.model} onChange={e => setEnrollForm(f => ({ ...f, model: e.target.value }))} className="kiosk-select" disabled={!enrollForm.manufacturer}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <KField label="Condition">
                  <select value={enrollForm.condition} onChange={e => setEnrollForm(f => ({ ...f, condition: e.target.value }))} className="kiosk-select">
                    {['New', 'Good', 'Fair', 'Poor'].map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </KField>
                <KField label="Building">
                  <select value={enrollForm.building} onChange={e => setEnrollForm(f => ({ ...f, building: e.target.value }))} className="kiosk-select">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={reset} className="px-5 rounded-xl border border-border text-muted-foreground hover:text-foreground text-sm">Cancel</button>
            </div>
          </div>
        )}
        ...
        {step === 'confirm' && asset && (
          <div className="w-full space-y-5">
            ...
            {mode === 'checkin' && assignee?.name && (
              <div className="rounded-xl border border-border px-4 py-3 flex items-center gap-3 bg-muted">
                <span className="text-muted-foreground text-sm">Returning from:</span>
            ...
            {mode === 'checkout' && (
              <div className="space-y-2">
                <label className="text-muted-foreground text-sm font-medium block">Assign to Person *</label>
                {selectedUser ? (
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-muted">
                    <div className="flex-1">
                      <p className="font-semibold">{selectedUser.firstName} {selectedUser.lastName}</p>
                      <p className="text-muted-foreground text-xs">{selectedUser.userType} · {selectedUser.email ?? 'No email'}</p>
                    </div>
                    <button onClick={() => { setSelectedUser(null); setPersonQuery(''); setTimeout(() => personRef.current?.focus(), 50) }}
                      className="text-muted-foreground hover:text-foreground text-xs px-2 py-1 rounded border border-border">Change</button>
                  </div>
                ) : (
                  <div className="relative">
                    <input ref={personRef} value={personQuery} onChange={e => setPersonQuery(e.target.value)}
                      ...
                      className="w-full text-lg px-4 py-3 rounded-xl border-2 outline-none bg-muted text-foreground"
                      style={{ borderColor: personQuery ? cfg.color : undefined }}
                      autoComplete="off" />
                    {personResults.length > 0 && (
                      <ul className="absolute top-full left-0 right-0 z-50 mt-1 rounded-xl border border-border overflow-hidden bg-popover shadow-2xl">
                        {personResults.map(u => (
                          <li key={u.id} onClick={() => { setSelectedUser(u); setPersonQuery(`${u.firstName} ${u.lastName}`); setPersonResults([]) }}
                            className="px-4 py-3 flex items-center gap-3 hover:bg-accent cursor-pointer border-b border-border last:border-0">
                            <div>
                              <p className="font-medium text-sm">{u.firstName} {u.lastName}</p>
                              <p className="text-muted-foreground text-xs">{u.userType} {u.idNumber ? `· ID: ${u.idNumber}` : ''} {u.email ? `· ${u.email}` : ''}</p>
                            </div>
                          </li>
                        ))}
                        <li onClick={() => setShowCreateUser(true)} className="px-4 py-3 text-sm font-medium cursor-pointer hover:bg-accent border-t border-border" style={{ color: cfg.color }}>
                          + Create new user
                        </li>
                      </ul>
                    )}
                    {personQuery.length >= 2 && personResults.length === 0 && (
                      <button onClick={() => setShowCreateUser(true)}
                        className="mt-2 w-full py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-border transition-colors">
            ...
            <div className="flex gap-3 pt-1">
              <button
                ...
                style={{ backgroundColor: cfg.color }}>
                {saving ? 'Saving…' : mode === 'checkout'
                  ? selectedUser
                    ? `Check Out → ${selectedUser.firstName} ${selectedUser.lastName}`
                    : personQuery.trim()
                      ? `Create & assign "${personQuery.trim()}"`
                      : 'Select a user…'
                  : 'Confirm Check In'}
              </button>
              <button onClick={reset} className="px-5 rounded-xl border border-border text-muted-foreground hover:text-foreground text-sm">Cancel</button>
        ...
        {step === 'success' && (
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground/50 text-sm font-mono">Resetting…</p>
            <button onClick={reset} className="mt-2 px-6 py-3 rounded-xl border border-border text-muted-foreground hover:text-foreground text-sm">Next →</button>
          </div>
        )}
      ...
      {showCreateUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setShowCreateUser(false)}>
          <div className="rounded-2xl border border-border p-6 w-full max-w-md space-y-4 bg-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <KField label="Type" className="col-span-2">
                <select value={newUser.userType} onChange={e => setNewUser(u => ({ ...u, userType: e.target.value }))} className="kiosk-select">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button onClick={() => setShowCreateUser(false)} className="px-4 rounded-xl border border-border text-muted-foreground hover:text-foreground text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}

      <p className="text-center pb-4 text-muted-foreground/30 text-xs font-mono tracking-widest">ENTER · ESC</p>

      <style>{`
        .kiosk-input { width: 100%; padding: 10px 14px; background: hsl(var(--muted)); color: hsl(var(--foreground)); border: 1px solid hsl(var(--border)); border-radius: 8px; outline: none; font-size: 14px; }
        .kiosk-input:focus { border-color: hsl(var(--ring)); }
        .kiosk-select { width: 100%; padding: 10px 14px; background: hsl(var(--muted)); color: hsl(var(--foreground)); border: 1px solid hsl(var(--border)); border-radius: 8px; outline: none; font-size: 14px; }
      `}</style>
    </div>
  )
}
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-border bg-muted">
      ...
      <div className="p-5 flex items-start justify-between">
        <div>
          <p className="text-muted-foreground text-xs font-mono uppercase tracking-widest mb-0.5">{label}</p>
          <h2 className="text-3xl font-bold font-mono">{asset.assetTag}</h2>
          <p className="text-muted-foreground text-base mt-0.5">{asset.manufacturer} {asset.model}</p>
          <p className="text-muted-foreground/50 text-xs font-mono mt-1.5">S/N: {asset.serialNumber}</p>
        </div>
        <div className="text-right space-y-1.5">
          ...
          {asset.room && <p className="text-muted-foreground/50 text-xs">{asset.building} · {asset.room.name}</p>}
          {assignee?.name && <p className="text-muted-foreground text-xs">→ {assignee.name}</p>}
        </div>
      </div>
    </div>
  )
...
function KField({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`space-y-1 ${className ?? ''}`}>
      <label className="text-muted-foreground text-xs font-medium block">{label}</label>
      {children}
    </div>
  )
}
