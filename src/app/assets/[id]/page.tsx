      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="text-sm space-y-1">
            ...
            {person && <p><span className="text-slate-500">Assigned to:</span> {person.name}{person.ou ? ` (${person.ou})` : ''}</p>}
            {(asset as any).checkedOutAt && <p><span className="text-slate-500">Checked out:</span> {new Date((asset as any).checkedOutAt).toLocaleString()}</p>}
            {(asset as any).providedAccessories?.length > 0 && (
              <p><span className="text-slate-500">Accessories:</span> {(asset as any).providedAccessories.join(', ')}</p>
            )}
            {(asset as any).secondaryTags?.length > 0 && (
              <p><span className="text-slate-500">Secondary tags:</span> <span className="font-mono text-xs">{(asset as any).secondaryTags.join(', ')}</span></p>
            )}
